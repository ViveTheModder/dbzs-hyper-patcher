...
public class Patch {
	public static boolean apply(RandomAccessFile iso, String patchArg, String[] patchArgs, String[] text, boolean print, JTextArea ta) throws IOException {
		int patchCnt = 0;
		...
		if (patchBools[0]) {
			...
			Main.displayResult(result, ta, print);
			...
			result = text[48] + text[66] + text[50];
			Main.displayResult(result, ta, print);
			//Ultimate Decisive Battle fix
			iso.seek(497591241); //go to address of incorrect voice line offset (set to "FF FF")
			iso.write(0x15); //replace with offset that points to 100 (correct voice line ID)
			iso.write(0);
			result = text[48] + text[90] + text[50];
			Main.displayResult(result, ta, print);
		} 
		if (patchBools[1]) {
		...
		if (patchBools[14]) {
			int[] transformBonusAddrs = {
				1010771389, 1011485805, 1012129213, 1012851117, //Android #17 addresses (main costumes)
				1013495229, 1014209645, 1014853053, 1015574957, //Android #17 addresses (damaged costumes)
				1074750445, 1075434445, 1076122605, 1076806605, //Buutenks
				1079212141, 1079851085, 1080494189, 1081133133  //Buuhan
			};
			for (int addrCnt = 0; addrCnt < transformBonusAddrs.length; addrCnt++) {
				iso.seek(transformBonusAddrs[addrCnt]);
				if (addrCnt < 12) iso.write(0x10); //Costume Repair fix for Android #17 and Buutenks
				else iso.write(0x14); //Costume Repair fix for Buuhan
			}
			result = text[48] + text[87] + text[50];
			Main.displayResult(result, ta, print);
		}
